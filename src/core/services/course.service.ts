...
export class CourseService extends BaseCoreService {
    constructor() {
        super();
    }

    public getList(option: IParamsRequestApi): void {
        this.requestApiWithAuth(API_DOCUMENT.course.getList, option)
            .then((res) => option.onSuccess(res))
            .catch((err) => option.onError(err));
    }

    public getById(option: IParamsRequestApi): void {
        this.requestApiWithAuth(API_DOCUMENT.course.getById, option)
            .then((res) => option.onSuccess(res))
            .catch((err) => option.onError(err));
    }
    ...
    public delete(option: IParamsRequestApi): void {
        this.requestApiWithAuth(API_DOCUMENT.course.delete, option)
            .then((res) => option.onSuccess(res))
            .catch((err) => option.onError(err));
    }
}
