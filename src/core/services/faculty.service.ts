...
export class FacultyService extends BaseCoreService {
    constructor() {
        super();
    }

    public getList(option: IParamsRequestApi): void {
        this.requestApiWithAuth(API_DOCUMENT.faculty.getList, option)
            .then((res) => option.onSuccess(res))
            .catch((err) => option.onError(err));
    }

    public getById(option: IParamsRequestApi): void {
        this.requestApiWithAuth(API_DOCUMENT.faculty.getById, option)
            .then((res) => option.onSuccess(res))
            .catch((err) => option.onError(err));
    }

    public create(option: IParamsRequestApi): void {
        this.requestApiWithAuth(API_DOCUMENT.faculty.create, option)
            .then((res) => option.onSuccess(res))
            .catch((err) => option.onError(err));
    }

    public update(option: IParamsRequestApi): void {
        this.requestApiWithAuth(API_DOCUMENT.faculty.update, option)
            .then((res) => option.onSuccess(res))
            .catch((err) => option.onError(err));
    }

    public delete(option: IParamsRequestApi): void {
        this.requestApiWithAuth(API_DOCUMENT.faculty.delete, option)
            .then((res) => option.onSuccess(res))
            .catch((err) => option.onError(err));
    }
}
